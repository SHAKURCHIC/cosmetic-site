...
export default function HomeSection({ onServiceClick }) {
    return (
        <div className="w-full flex flex-col items-center justify-center bg-[#F8F5F2]">
            {/* HERO */}
            <HeroSection />

            {/* Приветственный блок */}
            <section className="w-full flex flex-col items-center justify-center px-3 sm:px-4 mt-10 sm:mt-12">
                <motion.div
                    initial={{ opacity: 0, y: 40 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.8 }}
                    viewport={{ once: true }}
                    className="max-w-2xl w-full text-center bg-white/70 rounded-3xl p-4 sm:p-8 shadow-xl border border-[#EDE2CF] relative"
            ...
            {/* Галерея */}
            <section id="about-gallery" className="w-full flex flex-col items-center justify-center my-16 sm:my-20 px-2 sm:px-4">
