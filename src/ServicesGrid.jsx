    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-6xl mx-auto mt-10">
            {services.map((card) => (
                <div
                    key={card.id}
                    className="relative overflow-hidden rounded-2xl min-h-[260px] flex flex-col justify-end shadow-lg cursor-pointer group transition"
                    style={{
                ...
                >
                    ...
                    <div className="relative z-10 p-6 flex flex-col h-full">
                        <h3 className="text-2xl font-bold mb-2 text-white drop-shadow">{card.title}</h3>
                        {card.description && (
                            <ul className="mb-4">
                                {card.description.map((line, i) => (
                                    <li key={i} className="text-base text-white/90">{line}</li>
                                ))}
                            </ul>
                        )}
